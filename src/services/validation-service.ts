...
export class ValidationService {
    ...
    try {
      ...
      for (const file of sourceFiles) {
        try {
          ...
          if (content.includes('@web3modal') || content.includes('reown'))
            compatibility.imports.reownImports++;
      ...
      throw new Error(
        `Failed to check Wagmi compatibility: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  private async validatePackages(projectPath: string): Promise<any> {
    try {
      ...
      return {
        ...
        hasReownPackages: Object.keys(dependencies).some(
          (pkg) => pkg.includes('@web3modal') || pkg.includes('reown')
        ),
  ...
  private findConflictingPackages(dependencies: Record<string, string>): string[] {
    ...
    const potentialConflicts = ['@web3modal/wagmi', '@web3modal/siwe', '@web3modal/ethereum'];
  ...
  private getRequiredPackages(): string[] {
    return ['@getpara/react-sdk', 'wagmi', 'viem'];
  }
  ...
  private generateValidationRecommendations(results: any): string[] {
    ...
    if (results.packageValidation.conflictingPackages.length > 0) {
      recommendations.push(
        `Remove conflicting packages: ${results.packageValidation.conflictingPackages.join(', ')}`
      );
    }
  ...
  private generateCompatibilityRecommendations(compatibility: any): string[] {
    ...
    if (compatibility.wagmiHooks.found > 0) {
      recommendations.push(
        `Great! Found ${compatibility.wagmiHooks.found} Wagmi hooks that will continue to work with Para`
      );
    }
    ...
    if (compatibility.wagmiHooks.incompatible > 0) {
      recommendations.push(
        `Review ${compatibility.wagmiHooks.incompatible} potentially incompatible hooks`
      );
    }
  ...
  private calculateOverallStatus(results: any): string {
    const hasErrors = Object.values(results).some(
      (result: any) => result && typeof result === 'object' && 'error' in result
    );
    ...
    try {
      ...
      if (!hasParaModalInProvider) {
        if (paraProviderFound) {
          ...
          validationResults.paraModalCheck = {
            status: 'fail',
            details:
              '❌ CRITICAL: <ParaModal /> component missing in provider - modal will not appear',
          };
        } else {
          ...
          validationResults.paraModalCheck = {
            status: 'fail',
            details:
              '❌ CRITICAL: ParaProvider not found - please generate provider component first',
          };
        }
      }
      ...
      if (usesStringEnvironment && !usesEnvironmentEnum) {
        ...
        validationResults.environmentEnumCheck = {
          status: 'fail',
          details:
            '❌ CRITICAL: Using "development"/"production" strings instead of Environment enum',
        };
      ...
      try {
        ...
        for (const entry of entries) {
          ...
          if (
            ...
            if (
              entry.name.toLowerCase().includes('provider') ||
  ...
  private generateParaMigrationFixes(issues: string[]): string[] {
    ...
    for (const issue of issues) {
      if (issue.includes('ParaModal')) {
        ...
        fixes.push(
          'Location: In your provider component, add <ParaModal /> as a child of <ParaProvider>'
        );
  ...
  // New atomic validation methods
  private generateAtomicRecommendations(results: any): string[] {
    const recommendations: string[] = [];

    // Pre-flight recommendations
    if (results.preFlightValidation && !results.preFlightValidation.valid) {
      results.preFlightValidation.issues.forEach((issue: any) => {
        if (issue.fix) {
          recommendations.push(`❌ ${issue.message}: ${issue.fix}`);
        } else {
    ...
    if (results.postMigrationValidation && !results.postMigrationValidation.valid) {
      results.postMigrationValidation.issues.forEach((issue: any) => {
        if (issue.fix) {
          recommendations.push(`❌ ${issue.message}: ${issue.fix}`);
        } else {
          recommendations.push(`❌ ${issue.message}`);
    ...
    // Migration success recommendations
    if (results.migrationSuccessPercentage < 50) {
      recommendations.push('🚨 Migration is less than 50% complete - major issues detected');
    } else if (results.migrationSuccessPercentage < 90) {
      recommendations.push('⚠️ Migration is partially complete - some issues remain');
    } else {
      recommendations.push('✅ Migration is 90%+ complete - should be functional');
    }

    return recommendations;
  }

  private calculateAtomicStatus(results: any): string {
    if (results.error) return 'error';

    const migrationPercentage = results.migrationSuccessPercentage || 0;

    if (migrationPercentage >= 90) return 'ready';
    if (migrationPercentage >= 50) return 'warning';
    return 'needs_setup';
  }
}
